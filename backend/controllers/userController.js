...
const register = async (req, res) => {
    ...
    if (!account_type || !email || !password || !username || !full_name) {
        return res.status(400).json({
            message: 'Не все обязательные поля заполнены. Требуются: account_type, email, пароль, username, полное имя'
        });
    }
    ...
    try {
        ...
        const createUserResult = await client.query(
            ...
            [email, username, full_name, phone || null, birth_date || null, password, account_type]
        );
