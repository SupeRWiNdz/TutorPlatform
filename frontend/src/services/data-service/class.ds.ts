...
export class ClassDataService {
  ...
  createClass(session_id: string, name: string, link: string, description: string): Observable<any> {
    return this.http.post<any>(`${this.apiUrl}/class/new`, { session_id, name, link, description });
  }
  deleteClass(session_id: string, link: string): Observable<any> {
    return this.http.post<any>(`${this.apiUrl}/class/delete`, { session_id, link });
  }
  editClass(session_id: string, link: string, new_name: string, new_link: string, new_description: string): Observable<any> {
    return this.http.post<any>(`${this.apiUrl}/class/edit`, { session_id, link, new_name, new_link, new_description });
  }
  ...
  addMember(session_id: string, link: string, username: string, role: string): Observable<any> {
    return this.http.post<any>(`${this.apiUrl}/class/invite`, { session_id, link, username, role });
  }
